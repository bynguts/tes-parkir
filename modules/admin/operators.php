...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'delete') {
        ...
        if ($used->fetchColumn() > 0) {
            $error = 'Pelanggaran Constraint: Operator ini memegang log transaksi aktif.';
        } else {
            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-slate-100">
                        <th class="text-left px-6 py-4 text-[10px] font-bold uppercase tracking-widest text-slate-400 font-inter">Nama Personalia</th>
                        ...
                        <th class="text-left px-4 py-4 text-[10px] font-bold uppercase tracking-widest text-slate-400 font-inter">Kontak</th>
                    ...
                    <tr class="hover:bg-slate-50 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                <form method="POST" onsubmit="return confirm('Terminasi operator ini dari sistem?')">
                                    <?= csrf_field() ?>
...
<div id="addModal" class="hidden fixed inset-0 z-50 backdrop-blur-md bg-slate-900/40 flex items-center justify-center">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4">
        ...
        <form method="POST" class="p-6 space-y-4">
            ...
            <div><label class="block text-[10px] font-bold uppercase tracking-widest text-slate-400 font-inter mb-2">Nama Lengkap</label>
                <input type="text" name="full_name" required placeholder="Cth: Budiman Sudjatmiko" class="<?= $inputCls ?>"></div>
            ...
            <div><label class="block text-[10px] font-bold uppercase tracking-widest text-slate-400 font-inter mb-2">No. Telepon</label>
                <input type="tel" name="phone" placeholder="08xxxxxxxxxx" class="<?= $inputCls ?>"></div>
            <div class="flex gap-2 pt-2">
                <button type="button" onclick="document.getElementById('addModal').classList.add('hidden')" class="flex-1 bg-slate-100 text-slate-700 font-bold font-inter text-xs uppercase tracking-widest rounded-xl py-3">Batal</button>
                <button type="submit" class="flex-1 bg-slate-900 text-white font-bold font-inter text-xs uppercase tracking-widest rounded-xl py-3">Simpan</button>
            </div>
        </form>
    </div>
</div>
...
<div id="editModal" class="hidden fixed inset-0 z-50 backdrop-blur-md bg-slate-900/40 flex items-center justify-center">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4">
        <div class="flex items-center justify-between px-6 py-5 border-b border-slate-100">
            <div class="flex items-center gap-3"><span class="material-symbols-outlined text-slate-500">manage_accounts</span>
                <h2 class="font-manrope font-bold text-lg text-slate-900">Update Data Operator</h2></div>
            <button onclick="document.getElementById('editModal').classList.add('hidden')" class="text-slate-400 hover:text-slate-700"><span class="material-symbols-outlined">close</span></button>
        </div>
        <form method="POST" class="p-6 space-y-4">
            <?= csrf_field() ?>
            <input type="hidden" name="action" value="edit">
            <input type="hidden" name="operator_id" id="edit_id">
            <div><label class="block text-[10px] font-bold uppercase tracking-widest text-slate-400 font-inter mb-2">Nama</label>
            ...
            <div><label class="block text-[10px] font-bold uppercase tracking-widest text-slate-400 font-inter mb-2">Kontak Darurat</label>
                <input type="tel" name="phone" id="edit_phone" class="<?= $inputCls ?>"></div>
            <div class="flex gap-2 pt-2">
                <button type="button" onclick="document.getElementById('editModal').classList.add('hidden')" class="flex-1 bg-slate-100 text-slate-700 font-bold font-inter text-xs uppercase tracking-widest rounded-xl py-3">Batal</button>
                <button type="submit" class="flex-1 bg-slate-900 text-white font-bold font-inter text-xs uppercase tracking-widest rounded-xl py-3">Simpan</button>
            </div>
        </form>
    </div>
