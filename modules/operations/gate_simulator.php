...
<script>
    ...
    function processALPR(mode) {
        if (mode === 'entry') {
            ...
            // Regex for Indonesian plate format (B 1234 XYZ)
            const plateRegex = /^[A-Z]{1,2}\s\d{1,4}\s[A-Z]{1,3}$/;
            if (!plateRegex.test(plate)) {
                pushNotify('Invalid Format', 'Please enter a valid Indonesian plate (e.g., B 1234 XYZ).', 'error');
                return;
            }
            ...
            // If not a ticket code, must be a plate
            const plateRegex = /^[A-Z]{1,2}\s\d{1,4}\s[A-Z]{1,3}$/;
            if (!plateRegex.test(inputVal)) {
                pushNotify('Invalid Format', 'Please enter a valid Ticket Code or Indonesian Plate (e.g., B 1234 XYZ).', 'error');
                return;
            }
