...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'edit') {
        ...
        if (!$res_id || !$plate || !in_array($vtype, ['car', 'motorcycle']) || !$date_from) {
            ...
            // 1. Owner Name Validation (Min 3 chars, letters/spaces only)
            if (strlen($owner) < 3) {
                $error = "Full name must be at least 3 characters.";
            } else if (!preg_match("/^[a-zA-Z\s\']+$/", $owner)) {
            ...
            if (!$error) {
                ...
                if (!preg_match("/^(08|\+628)\d{8,12}$/", $clean_phone)) {
                    $error = "Invalid phone format (e.g. 081234567890).";
                } else {
                    $phone = $clean_phone;
                }
            }

            // 3. License Plate Validation (Indonesian standard)
            if (!$error) {
                if (!preg_match("/^[A-Z]{1,2}\s\d{1,4}\s[A-Z]{1,3}$/", $plate)) {
                    $error = "Invalid plate format (e.g. B 1234 XYZ).";
                }
            }
            ...
            try {
                ...
                $pdo->prepare("UPDATE reservation SET plate_number=?, slot_id=?, reserved_from=?, reserved_until=?, client_name=?, client_phone=? WHERE reservation_id=?")
                    ->execute([$plate, $slot_id, $date_from, $date_until, $owner ?: null, $phone ?: null, $res_id]);
...
<div class="px-10 py-10 max-w-[1600px] mx-auto space-y-10">
    ...
    <div id="editResModal" class="fixed inset-0 bg-black/80 backdrop-blur-md z-[1000] hidden items-center justify-center p-6">
        <div class="bg-surface border border-color w-full max-w-xl rounded-3xl overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-300">
            ...
            <form method="POST" class="p-8" id="editResForm">
                <?= csrf_field() ?>
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="text" name="plate_number" id="edit_plate"
                               ...
                               title="Standard Indonesian plate: [Area] [Number] [Suffix] (e.g., B 1234 XYZ)"
                    ...
                    <div class="grid grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="owner_name" id="edit_owner" required
                                   minlength="3" pattern="^[a-zA-Z\s\']+$"
                                   title="Full name (min 3 characters, letters/spaces only)"
                        ...
                        <div>
                            ...
                            <input type="tel" name="owner_phone" id="edit_phone" required
                                   pattern="^(08|\+628)\d{8,12}$"
                                   title="Indonesian phone number starting with 08 or +628"
                                   class="modal-input w-full border-2 border-transparent focus:border-brand rounded-xl px-4 py-3.5 text-[13px] font-bold font-manrope text-primary focus:outline-none transition-all">
                        </div>
                    </div>
                </div>
...
<script>
    ...
    // Client-side validation for Edit Reservation
    document.getElementById('editResForm').addEventListener('submit', function(e) {
        ...
        const name = document.getElementById('edit_owner').value.trim();
        const phone = document.getElementById('edit_phone').value.trim().replace(/[\s\-]/g, '');

        // Plate Regex
        if (!/^[A-Z]{1,2}\s\d{1,4}\s[A-Z]{1,3}$/.test(plate)) {
            e.preventDefault();
            pushNotify('Invalid Format', 'Plate must be standard Indonesian format (e.g. B 1234 XYZ)', 'error');
            return;
        }
        ...
        if (name.length < 3 || !/^[a-zA-Z\s\']+$/.test(name)) {
            e.preventDefault();
            pushNotify('Invalid Format', 'Name must be at least 3 characters and contain only letters.', 'error');
            return;
        }
        ...
        if (!/^(08|\+628)\d{8,12}$/.test(phone)) {
            e.preventDefault();
            pushNotify('Invalid Format', 'Phone must be a valid Indonesian number (starting with 08 or +628)', 'error');
            return;
        }
    });
</script>
