...
try {
    ...
    foreach ($active_trxs as $trx) {
        if ($trx['payment_status'] === 'unpaid') {
            $applied_rate = $trx['next_hour_rate'];
            $total_fee = ceil($trx['minutes_parked'] / 60) * $applied_rate;
            $upd = $pdo->prepare("UPDATE `transaction` SET check_out_time = NOW(), applied_rate = ?, total_fee = ?, payment_status = 'paid', is_force_checkout = 1 WHERE transaction_id = ?");
            $upd->execute([$applied_rate, $total_fee, $trx['transaction_id']]);
        } else {
    ...
    $cancelled_res = 0;
    // (Removed automatic cancellation of today's reservations during force checkout all)
