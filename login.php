...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!hash_equals(csrf_token(), $token)) {
        ...
        if ($_SESSION[$key]['count'] >= 5) {
            ...
            $stmt = $pdo->prepare("SELECT user_id, password_hash, role, full_name, operator_id FROM admin_users WHERE username = ? AND is_active = 1");
            ...
            if ($user && password_verify($password, $user['password_hash'])) {
                ...
                // If user is linked to an operator, check them in automatically
                if (!empty($user['operator_id'])) {
                    $_SESSION['staff_id'] = $user['operator_id'];
                    $_SESSION['staff_name'] = $user['full_name'];
                    
                    // End previous active shifts for this operator
                    $pdo->prepare("UPDATE shift_attendance SET is_active = 0, check_out_time = NOW() WHERE staff_id = ? AND is_active = 1")->execute([$user['operator_id']]);
                    
                    // Create new active shift attendance
                    $pdo->prepare("INSERT INTO shift_attendance (user_id, staff_id, check_in_time, is_active) VALUES (?, ?, NOW(), 1)")
                        ->execute([$user['user_id'], $user['operator_id']]);
                }
...
<head>
    ...
    <title>Enterprise Login — Parkhere</title>
    ...
    <div class="hidden lg:flex lg:w-1/2 brand-side flex-col justify-between p-20 relative">
        ...
        <div class="relative z-10">
            <div class="flex items-center gap-4 group">
                ...
                <h1 class="text-3xl font-manrope font-black text-white tracking-tight">Parkhere</h1>
            </div>
        </div>
    ...
    <div class="w-full lg:w-1/2 form-side flex flex-col items-center justify-center p-8 lg:p-24 relative overflow-hidden">
        ...
        <div class="w-full max-w-[420px] animate-slide">
            ...
            <div class="lg:hidden flex flex-col items-center mb-12">
                ...
                <h1 class="text-3xl font-manrope font-black text-[#0f172a] tracking-tight">Parkhere</h1>
            </div>
