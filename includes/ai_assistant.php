<!-- Parkhere: Floating Assistant UI -->
        ...
        <div class="px-4 py-3 flex items-center justify-between flex-shrink-0 bg-brand">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-white/70 text-[13px] font-medium font-inter" id="ai-view-status">Parkhere Assistant</p>
                </div>
            </div>
            ...
            <div class="p-3 bg-surface-alt border-t border-color flex-shrink-0">
                <form id="ai-chat-form" class="relative group flex items-center gap-2">
                    <input type="text" id="ai-user-input" 
                           placeholder="Ask something about Parkhere..." 
...
const ACTIVE_KEY = 'parkhere_active_session';
const HISTORY_KEY = 'parkhere_chat_history';
...
function switchView(view) {
    ...
    if (view === 'history') {
        ...
        status.textContent = 'Parkhere Assistant';
...
function showInitialGreeting() {
    ...
    setTimeout(() => {
        const initialText = "Hello, I'm **Cereza**. I manage the unified parking data for your enterprise network. Currently analyzing live feeds from **Mall Lippo Cikarang**. How can I assist you today?";
...
function loadAISession() {
    ...
    if (session) {
        ...
        const savedScroll = localStorage.getItem('parkhere_scroll_pos');
        if (savedScroll !== null) {
...
function toggleAIChat() {
    ...
    if (aiChatVisible) {
        ...
        localStorage.setItem('parkhere_scroll_pos', area.scrollTop);
...
document.getElementById('ai-chat-form').addEventListener('submit', async function(e) {
    ...
    // Menampilkan dan menyimpan chat user
    ...
    setTimeout(async () => {
        ...
        try {
            // ─── TARIK MEMORI DARI LOCALSTORAGE ───
            const sessionStr = localStorage.getItem(ACTIVE_KEY);
            let chatHistory = [];
            
            if (sessionStr) {
                const rawHistory = JSON.parse(sessionStr);
                // Ubah format dari {role, text} menjadi format standar AI {role, content}
                chatHistory = rawHistory.map(msg => ({
                    role: msg.role === 'bot' ? 'assistant' : msg.role,
                    content: msg.text
                }));
                
                // Karena pertanyaan user barusan udah terlanjur kesimpen, kita hapus dari history
                // Biar nggak dikirim dobel (karena dikirim terpisah di 'query')
                if (chatHistory.length > 0 && chatHistory[chatHistory.length - 1].content === query) {
                    chatHistory.pop();
                }
                
                // Batasi memori cuma 6 chat terakhir (biar server nggak berat)
                if (chatHistory.length > 6) {
                    chatHistory = chatHistory.slice(-6);
                }
            }

            // ─── KIRIM QUERY + HISTORY KE PHP ───
            const response = await fetch('<?= BASE_URL ?>api/ai_chat.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ 
                    query: query,
                    history: chatHistory // Memori ikut dikirim ke belakang
                })
            });
            
            const data = await response.json();
            if (data.error) throw new Error(data.error);
            // ENABLE ANIMATION: change 6th param to true
            appendMessage('bot', data.response, true, null, true, true, loadingId);
        } catch (err) {
            appendMessage('bot', '⚠️ **An error occurred:**\n\n' + err.message, false, null, true, true, loadingId);
        }
    }, 1200);
});
...
function appendMessage(role, text, save = true, ts = null, shouldScroll = true, animate = true, replaceId = null) {
    ...
    if (role === 'user') {
        ...
        // TYPEWRITER EFFECT for fresh bot responses
        if (animate && save) {
            let i = 0;
            const speed = Math.max(5, Math.min(20, 1000 / text.length)); // Adjust speed based on length
            const timer = setInterval(() => {
                const part = text.substring(0, i);
                bubble.innerHTML = marked.parse(part + (i < text.length ? '...' : ''));
                
                // Fix tables in partial render
                bubble.querySelectorAll('table').forEach(tbl => {
                    if (!tbl.parentNode.classList.contains('ai-table-scroll')) {
                        const tableWrapper = document.createElement('div');
                        tableWrapper.className = 'ai-table-scroll';
                        tbl.parentNode.insertBefore(tableWrapper, tbl);
                ...
                if (i >= text.length) {
                    clearInterval(timer);
                    bubble.innerHTML = marked.parse(text); // Final clean render
                    bubble.querySelectorAll('table').forEach(tbl => {
                        if (!tbl.parentNode.classList.contains('ai-table-scroll')) {
                            const tableWrapper = document.createElement('div');
                            tableWrapper.className = 'ai-table-scroll';
                ...
                i += 3; // Type 3 chars at a time for better pace
            }, speed);
        } else {
